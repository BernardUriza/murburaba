...
interface UseMurmubaraEngineReturn {
  ...
  toggleChunkPlayback: (chunkId: string, audioType: 'processed' | 'original') => void;
...
export function useMurmubaraEngine(
  ...
  const { autoInitialize = false, defaultChunkDuration = 8, ...config } = options;
  ...
  const toggleChunkPlayback = useCallback((chunkId: string, audioType: 'processed' | 'original') => {
