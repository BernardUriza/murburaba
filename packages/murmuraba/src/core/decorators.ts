  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        const logger = container.get<ILogger>(TOKENS.Logger);
        logger.error(
          `${target.constructor.name}.${propertyKey} failed after ${duration.toFixed(2)}ms`,
          error
        );
        throw error;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          if (attempt < maxAttempts) {
            ...
            const logger = container.get<ILogger>(TOKENS.Logger);
            logger.warn(
              `${target.constructor.name}.${propertyKey} failed, retrying in ${waitTime}ms (attempt ${attempt}/${maxAttempts})`
            );
