...
export function VadDisplay({ averageVad, vadData, chunkIndex, compact = false }: VadDisplayProps) {
  ...
  if (compact) {
    return (
      <div className={`${styles['vad-display']} ${styles.vadDisplayCompact}`} data-vad-level={vadLevel}>
        ...
        <div className={`${styles.vadDisplayMetric} ${styles.vadDisplayMetricFeatured}`}>
          ...
          <div className={styles['vad-metric__bar']}>
            <div 
              ...
              style={{ 
                width: `${vadPercentage}%`,
                background: vadLevel === 'high' ? 'linear-gradient(90deg, #10b981, #34d399)' :
                           vadLevel === 'medium' ? 'linear-gradient(90deg, #f59e0b, #fbbf24)' :
                           'linear-gradient(90deg, #ef4444, #f87171)',
                boxShadow: vadLevel === 'high' ? '0 0 10px rgba(16, 185, 129, 0.5)' :
                          vadLevel === 'medium' ? '0 0 10px rgba(245, 158, 11, 0.5)' :
      ...
      <div className={styles['vad-display__status']}>
        <span className={`${styles['vad-status']} ${styles[`vad-status--${vadLevel}`]}`}>
          {vadLevel === 'high' ? '✅ Strong Voice Activity' : 
           vadLevel === 'medium' ? '⚠️ Moderate Voice Activity' : 
           '❌ Low Voice Activity'}
        </span>
        {vadData && vadData.length > 0 && (
            ...
            {vadData.slice(-20).map((point, i) => (
              <div 
                ...
                style={{ 
                  ...
                  backgroundColor: point.vad > 0.7 ? '#10b981' : 
                                  point.vad > 0.3 ? '#f59e0b' : 
                                  '#ef4444'
                }}
              />
            ))}
