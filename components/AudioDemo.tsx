    ...
    <div className="bg-gray-900 rounded-xl p-6 space-y-6" data-testid="audio-demo">
      ...
      <div className="bg-gray-800 rounded-lg p-4">
        <div className="flex items-center justify-between">
          ...
          <span className={`font-semibold ${engineStatus === 'ready' ? 'text-green-400' : engineStatus === 'initializing' ? 'text-yellow-400' : engineStatus === 'error' ? 'text-red-400' : 'text-gray-400'}`} data-testid="engine-status">
            {engineStatus}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-800 rounded-lg p-4">
          ...
          {originalAudioUrl ? (
            <audio ref={originalAudioRef} controls src={originalAudioUrl} className="w-full" />
        ...
        <div className="bg-gray-800 rounded-lg p-4">
          ...
          {processedAudioUrl ? (
            <div className="space-y-2">
              <audio ref={processedAudioRef} controls src={processedAudioUrl} className="w-full" />
              <button onClick={downloadProcessedAudio} className="w-full px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors text-sm">💾 Descargar Audio Limpio</button>
            </div>
          ) : (<div className="h-12 bg-gray-700 rounded animate-pulse" />)}
        </div>
      </div>
      <div className="bg-gray-800 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={exportLogs} disabled={logs.length === 0} className="px-4 py-1 bg-gray-700 text-white rounded hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm">📥 Exportar Logs</button>
        </div>
        <div ref={logContainerRef} className="h-64 overflow-y-auto bg-gray-900 rounded p-3 font-mono text-xs text-gray-300" data-testid="audio-logs">
          {logs.map((log, index) => (
            <div key={index} className="mb-1">
              <span className="text-gray-500">[{new Date(log.timestamp).toLocaleTimeString()}]</span>
              {log.vad > 0 && (<><span className="text-blue-400"> Frame {log.frame}:</span><span className="text-green-400"> VAD={log.vad.toFixed(3)}</span><span className="text-yellow-400"> RMS={log.rms.toFixed(3)}</span></>)}
              <span className="text-gray-300"> {log.message}</span>
            </div>
          ))}
          {logs.length === 0 && (<div className="text-gray-500">Esperando procesamiento...</div>)}
        </div>
      </div>
      {logs.length > 0 && (
        <div className="bg-gray-800 rounded-lg p-4">
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-green-400">{(logs.filter(l => l.vad > 0).reduce((sum, l) => sum + l.vad, 0) / Math.max(1, logs.filter(l => l.vad > 0).length)).toFixed(3)}</p>
            </div>
            <div>
              <p className="text-gray-400">RMS Promedio</p>
              <p className="text-2xl font-bold text-yellow-400">{(logs.filter(l => l.rms > 0).reduce((sum, l) => sum + l.rms, 0) / Math.max(1, logs.filter(l => l.rms > 0).length)).toFixed(3)}</p>
            </div>
          </div>
        </div>
      )}
