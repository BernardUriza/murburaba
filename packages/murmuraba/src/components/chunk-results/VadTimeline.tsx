...
export const VadTimeline = React.memo(
  function VadTimeline({ vadData, chunkId }: VadTimelineProps) {
  ...
  (prevProps, nextProps) => {
    ...
    for (const i of checkPoints) {
      if (i >= 0 && i < prevProps.vadData.length) {
        if (
          prevProps.vadData[i]?.time !== nextProps.vadData[i]?.time ||
