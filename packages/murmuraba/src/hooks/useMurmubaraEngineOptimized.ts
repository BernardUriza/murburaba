...
import { engineReducer, initialState, EngineStoreState } from './engineReducer';
import {
  ...
  getEngineStatus,
  getDiagnostics as getEngineDiagnostics,
  onMetricsUpdate,
  getEngine,
} from '../api';
import {
...
export function useMurmubaraEngineOptimized(
  config: UseMurmubaraEngineOptions = {}
): any {
  const {
    autoInitialize = true,
  ...
  const audioRefs = useRef(new WeakMap<string, HTMLAudioElement>());
  const chunkRecordingsRef = useRef(new Map<string, any>());
  const metricsCleanupRef = useRef<(() => void) | null>(null);
  ...
  const audioConverterRef = useRef<any>(null);

  // Debounced timer update to reduce re-renders
  const updateRecordingTime = useMemo(
  ...
  const handleChunkProcessed = useCallback((chunk: ChunkMetrics) => {
  ...
  return {
    ...
    toggleChunkPlayback,
    toggleChunkExpansion: useCallback((id: string) => 
      dispatch({ type: 'TOGGLE_CHUNK_EXPANSION', payload: { id } }), []
    ),
...
function startFakeStreamingCycle(stream: MediaStream, originalStream: MediaStream, duration: number) {
