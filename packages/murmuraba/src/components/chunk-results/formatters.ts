...
export const calculateChunkStats = (
  chunks: Array<{ duration: number; isValid?: boolean; metrics: { processingLatency: number } }>
) => {
  ...
  const averageLatency =
    validChunks.length > 0
      ? validChunks.reduce((sum, chunk) => sum + chunk.metrics.processingLatency, 0) /
        validChunks.length
      : 0;
